...
def simulateNetwork(nSources, bSource, bSwitch, pGenRate, simTime):
    totDelay = pCount = inRate = outRate = 0
    packets = []
    ptotal = 10
    ...
    while not pq.empty() and curtime < simTime:
        ...
        if eid == 0:
            ...
            totDelay += packets[pid].sinkReachTime - packets[pid].created
            pCount = pid+1  # total packets reached sink
            #print("Packet {0} reached sink - {1}".format(pid, curtime))

    if pCount:
        avgDelay = totDelay / pCount
    if outRate:
        utilFactor = inRate / outRate
    return (avgDelay, utilFactor)
...
def main():
    ...
    while RATE <= MAXPGENRATE:
        avgDelay, utilFactor = simulateNetwork(
            NSOURCES, BSOURCE, BSWITCH, RATE, SIMTIME)
        y.append(utilFactor)
        x.append(avgDelay)
        print(RATE, utilFactor, avgDelay)
        RATE += 10
